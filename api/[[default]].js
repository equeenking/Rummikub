...
app.post('/room/create', (req, res) => {
  ...
  const player = {
    ...
    name: playerName,
    avatar: req.body.avatar || getRandomAvatar(),
    isBot: false,
    isReady: false,
    isHost: true
  };
...
app.post('/room/leave', (req, res) => {
  ...
  const playerIndex = room.players.findIndex(p => p.id === playerId || p.persistentId === playerId);
  if (playerIndex === -1) {
  ...
  room.playerMap.delete(player.persistentId);
...
app.get('/game/state', (req, res) => {
  ...
  const player = room.players.find(p => p.id === playerId || p.persistentId === playerId);
  if (!player) {
    return res.json({ success: false, message: '玩家不在房间中' });
  }

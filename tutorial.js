...
class GameGuideSystem {
    constructor() {
        ...
        this.guideSteps = [
            ...
            },
            {
                id: 'sort_button',
                title: '理牌',
                text: '点击"理牌"按钮可以智能整理手牌，自动识别有效牌组（顺子或刻子）。',
            ...
            },
            {
                id: 'draw_card',
                title: '摸牌',
                text: '如果无法出牌，点击"摸牌"按钮抽一张新牌。没破冰别气馁，继续摸牌等待机会！',
                selector: '#btn-draw',
                position: 'top',
        ...
        if (step.selector) {
            ...
            if (target) {
                ...
                if (tooltip) {
                    ...
                    const { position: bestPosition } = this.calculateBestPosition(
                        rect, tooltipWidth, tooltipHeight, vw, vh, margin
                    );

                    this.highlightElement(target, bestPosition);
                    this.showTooltip(step.title, step.text, target, bestPosition);
                    this.showArrow(target, bestPosition);
                }
            } else {
    ...
    showTooltip(title, text, element, position) {
        ...
        const { position: bestPosition, left, top } = this.calculateBestPosition(
            rect, tooltipWidth, tooltipHeight, vw, vh, margin
        );

        tooltip.className = `guide-tooltip ${bestPosition}`;
